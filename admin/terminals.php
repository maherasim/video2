...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_terminal'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO terminals (restaurant_id, terminal_id) VALUES (?, ?)");
        $stmt->bind_param("is", $restaurant_id, $terminal_id);
        $stmt->execute();
    }
    ...
    if (isset($_POST['update_terminal'])) {
        ...
        $stmt = $conn->prepare("UPDATE terminals SET restaurant_id = ?, terminal_id = ? WHERE id = ?");
        $stmt->bind_param("isi", $restaurant_id, $terminal_id, $id);
        $stmt->execute();
    }
}

// Fetch all terminals with restaurant names
$query = "
...
<div class="container">
    ...
    <!-- Edit Terminal Form (appears after clicking Edit) -->
